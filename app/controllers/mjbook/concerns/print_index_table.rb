module PrintIndexTable
 ...
 # HACK the helper is included in order to allow the formatting of data for processing by prawn
  ...
  include Printtables::PrintCreditnoteIndex
  include Printtables::PrintWriteoffIndex
  include Printtables::PrintMiscincomeIndex
  ...
  def table_data(index, data, rows, pdf)
    case index
      ...
      when 'creditnote' ; creditnote_data(data, rows)
      when 'writeoff' ; writeoff_data(data, rows)
      when 'writeoff' ; writeoff_data(data, rows)
      when 'miscincome' ; miscincome_data(data, rows)
    end
  end
  ...
  def column_widths(index)
    case index
      ...
      when 'creditnote' ; creditnote_column_widths
      when 'writeoff' ; writeoff_column_widths
      when 'miscincome' ; miscincome_column_widths
    end
  end
  ...
  def header_content(index, pdf)
    ...
    case index
      ...
      when 'creditnote' ; creditnote_headers
      when 'writeoff' ; writeoff_headers
      when 'miscincome' ; miscincome_headers
    end
  end
